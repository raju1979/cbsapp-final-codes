...
import { GrandQuestions } from '../grand-questions/grand-questions';
...
/*
  Generated class for the GrandLevelSelection page.

  See http://ionicframework.com/docs/v2/components/#navigation for more info on
  Ionic pages and navigation.
*/
@Component({
  selector: 'page-grand-level-selection',
  templateUrl: 'grand-level-selection.html',
  animations: [
    trigger('fadeInOut', [
...
export class GrandLevelSelection {
  ...
  maxLevelForThisCategory:number = 4;

  constructor(public navCtrl: NavController, public navParams: NavParams, private _userDataService: UserdataService, private _storage: Storage, public platform: Platform, private alertCtrl: AlertController,private _network:Network, private _file:File,private loadingCtrl: LoadingController) {
  ...
  ionViewWillEnter() {
    ...
    //   this._storage.get("grand_" + this.packageIdPasssed).then((val) => {
    //     console.log(val)
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad GrandLevelSelectionPage');
  };//
      ...
      .then((val) => {
        if(val == null){
          ...
          if(idIndex > -1){//if online level package found in local
            console.log(val[idIndex].max_grand_level);
            let receivedLevelNum:number = (+val[idIndex].max_grand_level) * 1;
            ...
            this.createNewGrandTestBaseUponOnlineLevels(parseInt(val[idIndex].max_grand_level));
  ...
  createNewGrandTest(levelNum: number) {
    ...
    console.log("passed level is",levelNum)
    ...
    if (this.platform.is("mobile") && !this.isGuestUser) {
      ...
      this._file.readAsText(this.fs, this.mainDirectoryName + "/" + this.packageIdPasssed + "/grandpackage/" + jsonFileName).
        then((data: any) => {
          this.loader.dismiss();
          let grandJson = JSON.parse(data);
          console.log('questions json from local read::', grandJson);
          this.submitDataToStorageAndNavigate(grandJson, suppliedLevel);
        })
        .catch((err) => {
          this.loader.dismiss();
      ...
      if(this.platform.is('mobile') && this._network.type=='none'){
          ...
          this._userDataService.getGrandQuestionsFromServer(this.packageIdPasssed, levelNum)
          .subscribe(
          (response) => {
            this.loader.dismiss();
            ...
            if (status == 200) {
              ...
              console.log('id passed',this.packageIdPasssed,' from server ',responseData)
  ...
  submitDataToStorageAndNavigate(responseData: any, suppliedLevel: any) {
    ...
    console.log(data);
    this._userDataService.setGrandQuestionDataIndexedDb(this.packageIdPasssed, data);//set data into indexedDb
    setTimeout(() => {
      this.currentLevel = 0;
      this.navCtrl.push(GrandQuestions, { id: this.packageIdPasssed,guestUser:this.isGuestUser });
    }, 500)
  ...
  navigateToImageTestScreen(btnLevel: number): void {
    ...
    if(this.allLevelCleared){
      ...
      if (this.previousTestExists) {
        ...
        this.createNewGrandTest(btnLevel);
      }//end inner if
    }//end outer if
  ...
  createNewGrandTestBaseUponOnlineLevels(levelNum: number) {
    ...
    if (this.platform.is("mobile") && !this.isGuestUser) {
      ...
      if(this.platform.is('mobile') && this._network.type=='none'){
          ...
          this._userDataService.getGrandQuestionsFromServer(this.packageIdPasssed, suppliedLevel)
          .subscribe(
          (response) => {
            this.loader.dismiss();
            let status = response.status;
    ...
    this._storage.get("user_master")
      .then((data) => {
        if(data == null){
          ...
          this._userDataService.deleteAllRowsForUserFromGrandScoreMaster(userData)
            .subscribe(
              (response) => {
                this.loader.dismiss();
                let status = response.status;
